...
// (see the documentation for MvsVersion). In particular, MVS does not
...
type Reqs interface {
	...
	Required(m MvsVersion) ([]MvsVersion, error)
...
type UpgradeReqs interface {
	...
	Upgrade(m MvsVersion) (MvsVersion, error)
}
...
type DowngradeReqs interface {
	...
	// Previous returns the version of m.PackageName immediately prior to m.Version,
	// or "none" if no such version is known.
	Previous(m MvsVersion) (MvsVersion, error)
}
...
func buildList(targets []MvsVersion, reqs Reqs, upgrade func(MvsVersion) (MvsVersion, error)) ([]MvsVersion, error) {
	...
	var (
		...
		upgrades = map[MvsVersion]MvsVersion{}
		errs     = map[MvsVersion]error{} // (non-nil errors only)
	)

	// Explore work graph in parallel in case reqs.Required
	// does high-latency network operations.
	var work par.Work[MvsVersion]
	for _, target := range targets {
		work.Add(target)
	}
	work.Do(10, func(m MvsVersion) {

		var required []MvsVersion
		var err error
		if !m.IsNone() {
		...
		if u != m {
			upgrades[m] = u
			required = append([]MvsVersion{u}, required...)
		}
	...
	if len(errs) > 0 {
		errPath := g.FindPath(func(m MvsVersion) bool {
			return errs[m] != nil
		})
		if len(errPath) == 0 {
		...
		// isUpgrade := func(from, to MvsVersion) bool {
...
func Req(mainModule MvsVersion, base []string, reqs Reqs) ([]MvsVersion, error) {
	...
	var postorder []MvsVersion
	reqCache := map[MvsVersion][]MvsVersion{}
	reqCache[mainModule] = nil

	var walk func(MvsVersion) error
	walk = func(m MvsVersion) error {
		_, ok := reqCache[m]
		if ok {
			return nil
		}
...
func Downgrade(target MvsVersion, reqs DowngradeReqs, downgrade ...MvsVersion) ([]MvsVersion, error) {
	...
	var (
		added    = make(map[MvsVersion]bool)
		rdeps    = make(map[MvsVersion][]MvsVersion)
		excluded = make(map[MvsVersion]bool)
	)
	var exclude func(MvsVersion)
	exclude = func(m MvsVersion) {
		if excluded[m] {
			return
		}
		excluded[m] = true
	...
	var add func(MvsVersion)
	add = func(m MvsVersion) {
		if added[m] {
			return
		}
		added[m] = true
	...
	downgraded := make([]MvsVersion, 0, len(list)+1)
...
List:
	...
	return BuildList([]MvsVersion{target}, &override{
		target: target,
		list:   downgraded,
		Reqs:   reqs,
	})
...
func (r *override) Required(m MvsVersion) ([]MvsVersion, error) {
	if m == r.target {
		return r.list, nil
	}
